...
3. **Безопасность**: Правила гарантируют, что:
   ...
   - Счётчик `currentUses` отображается в админ-панели на основе сохранённых заказов
...
**Важно**: Счётчик `currentUses` не обновляется автоматически при создании заказов. Для обновления счётчика:
- Вручную увеличивайте значение через админ-панель (используя кнопку обновления)
- Или реализуйте Cloud Function для автоматического подсчёта на основе заказов
- Или добавьте скрипт для периодической синхронизации `currentUses` с реальным количеством заказов
