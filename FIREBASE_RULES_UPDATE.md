...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    match /promoCodes/{promoCodeId} {
      ...
      // - Система может увеличивать currentUses при применении промокода
...
2. **Доступ к промокодам**:
   ...
   - **Создание/Удаление (create/delete)**: Только администратор
   - **Обновление (update)**: 
     - Администратор может обновлять всё
     - Система может увеличивать счётчик `currentUses` при применении промокода

3. **Безопасность**: Правила гарантируют, что:
   - Обычные пользователи не могут изменять промокоды
   - Система может только увеличивать счётчик использований на 1
   - Все остальные поля могут изменять только администраторы
...
5. Проверьте, что счётчик использований увеличивается
...
```typescript
{
  ...
  currentUses: number;       // Текущее число использований
