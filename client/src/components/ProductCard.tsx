...
export default function ProductCard({
  ...
  return (
    ...
    >
      ...
      <div className={`p-4 space-y-3 ${isNewYear ? 'bg-gradient-to-b from-red-50 via-white to-red-50 dark:from-blue-900 dark:via-gray-900 dark:to-blue-900' : 'bg-gradient-to-b from-white via-pink-50/20 to-white dark:from-gray-800 dark:via-gray-900 dark:to-gray-800'} relative ${!isNewYear && 'caramel-drip'}`}>
        <h3 className={`font-medium text-sm line-clamp-2 min-h-[2.5rem] ${isNewYear ? 'text-red-900 dark:text-red-200' : 'text-gray-900 dark:text-white'}`} data-testid={`text-product-name-${id}`}>
          {name}
        </h3>
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            {hasDiscount ? (
              <>
                <span className={`text-xl font-bold text-transparent bg-clip-text ${isNewYear ? 'bg-gradient-to-r from-red-600 via-yellow-500 to-red-500' : 'bg-gradient-to-r from-pink-600 via-primary to-purple-600'} drop-shadow-sm`} data-testid={`text-sale-price-${id}`}>
                  {salePrice}₽
                </span>
              ...
              <span className={`text-xl font-bold text-transparent bg-clip-text ${isNewYear ? 'bg-gradient-to-r from-red-600 via-yellow-500 to-red-500' : 'bg-gradient-to-r from-pink-600 via-primary to-purple-600'} drop-shadow-sm`} data-testid={`text-price-${id}`}>
                {price}₽
              </span>
            )}
          </div>
        ...
        {isOutOfStock ? (
          ...
          <Button
            className={`w-full rounded-full squish-active text-white font-semibold text-sm py-6 ${isNewYear ? 'bg-gradient-to-r from-red-500 via-yellow-400 to-red-600 hover:from-red-600 hover:via-yellow-500 hover:to-red-700' : `gummy-button bg-gradient-to-r from-primary via-pink-500 to-accent hover:from-pink-600 hover:via-primary hover:to-purple-500 ${isAdding ? 'jelly-add-animation' : ''}`}`}
